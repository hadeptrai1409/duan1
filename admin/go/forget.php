...
$user_all = list_all_user();
 ...
 if(isset($_POST['btn-submit']) ) {
	...
	$result_user = list_all_user();
	foreach($result_user as $us){
		$user[]=$us['username'];
	}
	foreach($result_user as $emails){
		$emails[]=$us['email'];
	}

	if(isset($username) && isset($email)){
		if(in_array($username,$user) && in_array($email,$emails)){
			$data = array(
				'password' => password_hash("$password", PASSWORD_DEFAULT)
			);
			update('user',$data,'username',$username);
		}else{
			$error =  "Không tồn tại username hoặc email trong tài khoản";
		}
	}
	}
?>
...
<div class="login">
    ...
    <form method="POST" action="">
        ...
        <button type="submit"  name="btn-submit"  class="btn btn-primary btn-block btn-large">Gửi</button>
    </form>
</div>
